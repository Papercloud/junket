...
describe Junket::FilterConditionsController, type: :controller do
  ...
  describe 'PUT /junket/filter_conditions/:id' do
    it 'does not allow editing a condition on a public template' do
      pending 'KG broke it'
      public_template = create(:junket_action_template, access_level: :public, owner_id: 1, owner_type: 'AdminUser')
      filter_condition = create(:junket_filter_condition, action_template: public_template)

      put :update, action_template_id: public_template.id, id: filter_condition.id, filter_condition: { value: 'new value' }
      expect(response.response_code).to eq 403
    end

    it "does not allow editing a condition on someone else's private template" do
      pending 'KG broke it'
      private_template = create(:junket_action_template, access_level: :private, owner_id: 2, owner_type: current_user.class.name)
      filter_condition = create(:junket_filter_condition, action_template: private_template)

      put :update, action_template_id: private_template.id, id: filter_condition.id, filter_condition: { value: 'new value' }
      expect(response.response_code).to eq 403
    end
